...
- **Terminal window title**: `cc-agent-session` also sets the window title to `<agent> · <repo> · <start>`, so windows are distinguishable from the taskbar. The widget already runs on every statusline render, so this needs no hook; the escape is written straight to `/dev/tty`, because widget and hook stdout both belong to Claude Code rather than the terminal. `CC_AGENT_SESSION_NO_TITLE=1` keeps the statusline segment but drops the title, `cc-agent-session --title` is title-only (for a `SessionStart`/`UserPromptSubmit` hook, e.g. if Claude Code's own title overwrites it), and `CC_TITLE_TTY=<file>` redirects the escape to a file for inspection.

...
- `cc-agent-session`: companion ccstatusline widget printing `<agent> · <session start>`, prefixed with 📌 when `tbe freeze` has pinned the session (same `ccsessions-frozen.json`). It also sets the terminal window title to `<agent> · <repo> · <start>` (written straight to `/dev/tty`, since widget/hook stdout belongs to Claude Code); `CC_AGENT_SESSION_NO_TITLE=1` opts out, and `cc-agent-session --title` does title-only for use as a hook.
